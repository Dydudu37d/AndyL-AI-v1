...
class AIComputerController:
    ...
    def __init__(self):
        ...
        # 初始化AI想法显示管理器
        self.ai_thought_display = get_ai_thought_display()
    ...
    def _send_to_ollama(self, screenshot_path: str, prompt: str = "",need_commands:bool=True) -> Optional[str]:
    ...
    def _build_ollama_prompt(self, original_prompt: str) -> str:
        ...
        system_prompt += "- t_text: 输入文本，例如t_hello将输入hello\n"
    ...
    def mock_ai_response(self, screenshot_path: str, prompt: str = "") -> str:
        ...
        elif "探索桌面" in prompt_lower or "电脑內有什麼" in prompt_lower:
            # 模拟探索桌面环境的操作
    ...
    def run_single_cycle(self, prompt: str = "", use_mock: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            # 显示AI开始思考
            self.ai_thought_display.display_thought("开始电脑控制循环...")
            
            # 1. 捕获屏幕
            self.ai_thought_display.display_thought("正在捕获当前屏幕内容...")
            screenshot_path = self.capture_screen()
            if not screenshot_path:
                self.ai_thought_display.display_thought("错误: 屏幕捕获失败", clear_after=5)
            ...
            # 2. 获取AI指令
            self.ai_thought_display.display_thought("正在分析屏幕内容并生成控制指令...")
            if use_mock:
            ...
            if not commands:
                self.ai_thought_display.display_thought("错误: 获取AI指令失败或指令为空", clear_after=5)
            ...
            # 显示AI决定执行的操作
            self.ai_thought_display.display_thought(f"决定执行的操作: {commands}")
            
            # 3. 执行指令
            self.ai_thought_display.display_thought(f"正在执行操作...")
            ...
            total_count = execution_results.get("total_commands", 0)
            
            if success_count == total_count:
                self.ai_thought_display.display_thought(f"操作执行成功: 完成了 {success_count}/{total_count} 个命令", clear_after=10)
            else:
                failed_commands = execution_results.get("failed_commands", [])
                self.ai_thought_display.display_thought(f"操作部分成功: 完成了 {success_count}/{total_count} 个命令，失败命令: {failed_commands}", clear_after=10)
            ...
            self.ai_thought_display.display_thought(f"错误: {error_msg}", clear_after=5)
    ...
    def run_continuous(self, prompt: str = "", cycles: int = 10, interval: float = 2.0, use_mock: bool = True) -> list[Dict[str, Any]]:
        ...
        # 显示开始连续控制循环的想法
        self.ai_thought_display.display_thought(f"开始连续电脑控制循环，将执行 {cycles} 个操作循环")
        ...
        try:
            for i in range(cycles):
                ...
                # 显示当前循环的进度
                self.ai_thought_display.display_thought(f"执行控制循环 {i+1}/{cycles}")
                ...
                # 等待指定的间隔时间
                if i < cycles - 1:
                    self.ai_thought_display.display_thought(f"循环 {i+1} 完成，等待 {interval} 秒进行下一个循环...")
                    time.sleep(interval)
        except KeyboardInterrupt:
            self.ai_thought_display.display_thought("用户中断了连续控制循环", clear_after=5)
            logger.info("用户中断了连续控制循环")
        except Exception as e:
            self.ai_thought_display.display_thought(f"错误: 连续控制循环执行失败", clear_after=5)
        ...
        # 显示连续控制循环的最终结果
        self.ai_thought_display.display_thought(f"连续控制循环完成，共执行 {len(all_results)} 个循环，成功 {success_count} 个", clear_after=10)
